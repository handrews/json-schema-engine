...
| --- | -------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| S1  | Separate annotations from keyword dependency information | Delivered: `AnnotationRecord` (`ctx.annotate()`, the keyword's own value) and `DependencyRecord` (`ctx.produce()`/`ctx.visible()`) as separate stores in both tiers; applicator keywords produce dependency data only; `UndeclaredProductionError` guards `analyze().produces`. The compiler impact estimate for S2/S3 is in the [reconciliation investigation](investigations/ietf-draft-03-reconciliation.md#compiled-list-mode-relevance).                                                                                       |
| S2  | Implement keyword/schema relevance and irrelevance       | Delivered: one generic rule in `evaluateKeyword` drops an accepting keyword's rejecting sub-evaluation errors (kept as `droppedErrors` when tracing); relevant annotations are a valid run's root survivors; non-verbose output omits irrelevant records and prunes empty units, modern verbose marks them, 2020-12 Verbose unchanged; `if`/`then`/`else` are separate keyword results; dependency data only from an accepting producer; compiled list mode at parity. Fixtures: `relevance.test.ts`, `relevance-compiled.test.ts`. |
| S3  | Reconcile short-circuit and retention analysis           | Delivered as DESIGN.md §4 rule 7: the interpreter never short-circuits, compiled flag mode short-circuits only in verdict-only regions, list mode runs every branch; verdict invariance across output modes and annotation settings is pinned in `relevance.test.ts`.                                                                                                                                                                                                                                                               |
