...
export const VOCAB_CONTENT = "https://json-schema.org/draft/2020-12/vocab/content";
...
const contentVocabulary = Object.fromEntries(
  ["contentMediaType", "contentEncoding", "contentSchema"]
    .map((name) => [name, annotationOnly(`${VOCAB_CONTENT}#${name}`)]),
);
...
export function registerStandardDialects(registry: DialectRegistry): void {
  ...
  registry.registerVocabulary(VOCAB_FORMAT_ANNOTATION, formatAnnotationVocabulary);
