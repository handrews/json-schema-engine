...
| M3 | References hardened: `$dynamicRef`/dynamic scope (D8), `$vocabulary` processing, remote-resource loaders, metaschema validation policy | judgment | `dynamicRef`, `anchor`, `vocabulary`, `refRemote` (suite remotes served by test-kit) green; local Bowtie run ≥ 99% |
