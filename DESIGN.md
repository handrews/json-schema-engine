...
**Semantic-target notice (2026-09-05):** the IETF draft-03 record model
(exact-value annotations separate from keyword dependency information, §12.2
relevance) and the output model of
[ADR 0003](docs/planning/next-steps/decisions/0003-output-levels-and-orthogonal-controls.md)
(formats by name, three levels, orthogonal controls) are implemented in the
interpreter; D4-D6 and §4 below state them. The release-first plan in
[docs/planning/next-steps](docs/planning/next-steps/README.md) governs what
follows: cross-tier parity for the structured formats and the registry
snapshot (Phase 3), then the release gate.
...
| D4  | Keyword communication                 | **Frame-scoped record channel** (§4) with two record kinds: annotation records (the keyword's own value; output) and dependency records (computed data for other keywords; never output). Schema application pushes a frame; records merge to the parent frame only on success. Annotation dropping, in-place-applicator visibility for `unevaluated*`, and cousin-invisibility all fall out of the one scoping rule.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Prototype: 852 suite cases + channels tests pass with no per-keyword special cases                                                                                                                                                                                                                                     | A keyword needing a third record kind → add a kind, not a mechanism                                                                       |
| D5  | Annotation selection                  | Annotation selection (`annotations: false \| true \| selection`), a control independent of output format and level (ADR 0003): allow-lists by keyword name and vocabulary URI, deny-lists (`excludeKeywords`/`excludeVocabularies`) subtracted after them, a `keep` predicate over the rendered native unit. Internal consumers always see the channel regardless of selection. The interpreter elides annotation records at annotate time when the selection rules the keyword out — at every output level, since a deselected keyword is never rendered relevant or not (the `keep` predicate runs only at render, so recording its superset is required) — and dependency records when no registered keyword consumes them (registry-accumulated `StaticFacts.consumes`). The compiler specializes the lists into the artifact and runs `keep` in the wrapper. **Consumers MUST declare `consumes` and producers `produces` in `analyze()`** — `ctx.visible()` throws `UndeclaredConsumptionError` for undeclared ids and `ctx.produce()` throws `UndeclaredProductionError` for an undeclared producer, so a violation is loud, never a silently empty channel. Verified by an elision-on/off differential over all four dialect suites.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Spec-sanctioned (ANALYSIS.md §5 quotes); SPIKE.md: policy-specialized artifact 32M ops/s vs 51M with annotations fully elided — cost proportional to retention. Interpreter-side elision buys allocation/GC relief but only ~2% throughput (interpretation dominates); the CPU win is D9b's, in the compiler           | —                                                                                                                                         |
| D6  | Output                                | Output formats are selected by name (ADR 0003): `flag`, `basic`, `detailed`, `verbose` (IETF draft-03 §13) and `list`, `hierarchical` (machines-oriented output proposal), each fixing its document structure and field vocabulary; three levels — minimal (`flag`, the default), relevant, verbose (`verbose`; `list`/`hierarchical` with `verbose: true`, which keeps every unit and marks irrelevant records) — and orthogonal controls (`annotations`, `errorParams`, `positions`, `trace`) that act on the flat `Result.errors`/`Result.annotations` surface, present on every non-flag format, whose units carry the native names `evaluationPath`/`schemaLocation`/`inputLocation`; documents never carry extension data. An unsupported combination throws `OutputOptionsError` before evaluation. The flat surface plus a located tree of applications (`RenderInput`: string locations, per-keyword verdicts, index lists into the units) feeds every renderer, whichever tier produced it; the draft-03 trees render one node per keyword evaluation. Evaluation path tracked natively (constant in compiled code; push/pop + lazy string materialization in the interpreter). The relevant level of `list`/`hierarchical` is the default and applies relevance to the proposal's structures, which have none: irrelevant records are omitted and `hierarchical` prunes units left empty — a deliberate departure from the proposal, whose pruning is opt-in; `verbose: true` is the unpruned structure. Relevance is read per record from `droppedErrors`/`droppedAnnotations`, and in `verbose` from `valid` along the whole path from the root, never from one node's `valid`.                                                                                                       | The engine exists partly because both incumbents refuse half of this                                                                                                                                                                                                                                                   | Output spec changes upstream                                                                                                              |
| D7  | Async boundary                        | Loading/registration async (loaders for file/http/media types); **`compile` and `evaluate` are sync**. **Amended (ADR 0005, 2026-09-23):** the load loop takes one pending reference at a time, so a loader that throws leaves the rest of the queue for the next `load`/`loadSchema`; `validateSchemas` runs before registration on every path, loader-fetched documents included. **Amended (ADR 0006, 2026-09-23):** `validateSchemas` runs inside registration, per resource, between the walk and the commit; the async paths assemble a dialect an embedded resource declares (`UnknownDialectError.dialectUri`) and retry the atomic registration; a self-describing metaschema assembles before it registers.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Hyperjump's async-generator compile pipeline is a real tax; resource I/O is the only inherent async                                                                                                                                                                                                                    | —                                                                                                                                         |
| D8  | Dynamic scope                         | Full 2020-12 `$dynamicRef` semantics: dynamic scope = stack of entered schema resources; resolution requires the anchor lexically at the target, then rebinds to the outermost dynamic scope containing a matching `$dynamicAnchor`. 2019-09 `$recursiveRef/(Anchor)` as the degenerate case. Compiler resolves a `$dynamicRef` site at plan time when every path that can reach it yields the same target (a per-root dataflow over the unit graph; ADR 0004, 2026-09-21) and compiles the target statically; a site whose target can differ by path becomes a dynamic island → interpreter trampoline with the compiled caller's scope stack; `$recursiveRef` goes through the same analysis (ADR 0004 amendment, 2026-09-21).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | The single thing AJV got structurally wrong (fragment-only, root-registry, first-write-wins); prior art in the user's oas-tree-viewer `dynamicScope.ts` strict-winner analysis                                                                                                                                         | —                                                                                                                                         |
...
| D11 | Draft support                         | Native in core: 2020-12, IETF drafts, 2019-09, draft-07, **draft-06** (owner amendment 2026-07-06: draft-06 is a compatible subset of draft-07, so it rides along in M4). draft-04 as a separately packaged legacy dialect module (M10). **Amended (2026-07-07):** separate packaging retained but RE-JUSTIFIED — the driver is no longer AJV migration (`ajv-draft-04` users are not a wedge; AJV's draft-04 support is unproblematic in production) but the owner's downstream project needing draft-04 alongside later drafts. Packaging never limited capability: a separately packaged dialect registers into the same engine and coexists with every draft in one registry (the D18 extractor + `refIgnoresSiblings` hooks were built for exactly this). New rationale: (a) core's native-drafts story stays spec-clean (no vendored draft-04 metaschema, `id` extractor, boolean `exclusive*`, or draft-04 `integer` semantics in every consumer's bundle); (b) the package is the reference proof of the PUBLIC dialect-authoring surface — the same path the downstream project's OAS 3.0 Schema dialect (draft-04-based + `nullable`/`discriminator`/`xml`) will follow. M10 MUST build against the public registry API only; any needed private hook is a core extension-surface bug to fix, never a reason to fold draft-04 into core. The downstream project is the integration point (depends on the package, registers the dialect in its setup path).                                                                                                                                                                                                                                                                                                                              | ANALYSIS.md §7.7: market center of mass is draft-07+; only draft-04's genuinely different syntax (`id`, boolean `exclusive*`) stays out of core                                                                                                                                                                        | ajv-compat adoption data shows heavy draft-04 demand                                                                                      |
...
| D16 | Packaging                             | Monorepo, npm workspaces; packages under a scope TBD (owner decision, M0 blocker): `core`, `compiler`, `formats`, `dialect-draft04` (named `dialect-legacy` until M10 landed under the draft-04-specific name), `ajv-compat`, `test-kit` (suite runner + differential fuzzer), `bench`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Mirrors the staging in §6                                                                                                                                                                                                                                                                                              | —                                                                                                                                         |
| D18 | Per-dialect identifier syntax         | Identifier extraction is **dialect data**, not keyword behavior: `DialectOptions.identifiers` is an `IdentifierExtractor` `(schemaObject) → { baseId?, anchors?, dynamicAnchor?, recursiveAnchor? }`, consumed by the registration walk and by JSON Pointer navigation (which uses the target document's dialect). Shipped extractors: 2020-12 (`$id`/`$anchor`/`$dynamicAnchor`), 2019-09 (`$id`/`$anchor`/boolean `$recursiveAnchor`, root-effective), legacy draft-07/06 (`$id` only; plain-fragment `$id` mints an anchor; **`$ref` present ⇒ no identifiers at all**, which yields the suite's sibling-`$id` suppression). Companion option `refIgnoresSiblings` (draft-07/06): a schema object with `$ref` evaluates only `$ref` — siblings act as if absent ("ignores", not "overrides": nothing about their values changes, they are simply not there for evaluation). `$vocabulary`-assembled dialects inherit the extractor of whichever core vocabulary they include. **Amended (ADR 0005, 2026-09-23):** a `baseId` an extractor returns must name a resource — empty, `#`, or fragment-bearing (an empty trailing fragment excepted) is `InvalidIdentifierError`, at the root and embedded alike; one resource holds one schema and one anchor name names one object per resource (`DuplicateResourceError`/`DuplicateAnchorError`), an equal copy being no duplicate; replacement is explicit `unregister` then `register`. **Amended (ADR 0006, 2026-09-23):** the walk rebinds the dialect at an embedded `$id` to the resource's own `$schema` — the enclosing extractor decides the boundary, the inner one the contents — and pointer navigation switches extractor at each boundary; under `refIgnoresSiblings` a `$ref`'s siblings contribute nothing at registration either. | draft-07/06 in core (D11) need identifier and `$ref` semantics that no keyword-level mechanism can express; extractors keep the registry draft-agnostic                                                                                                                                                                | A draft whose identifiers depend on evaluation state (none known)                                                                         |
...
stack), `ctx.annotate()`, `ctx.produce(data)`, `ctx.visible(ids)`,
`ctx.error(message, params)`. See `prototype/engine.ts`
...
harvest direction. (A `$dynamicRef` target resolved at plan time — ADR
0004 — is an ordinary static edge, not code inside an island, so it leaves
...
and flat annotation collection (`compileList` `annotations`
selection): produce IR renders into an annotation array with
...
the selection's allow/deny lists elide statically at serialize time (the
per-output-config artifact key, D5), the `keep` predicate runs at the
wrapper over the native unit, and islands harvest their root-frame
...
Artifact key = (root URI × registry snapshot × annotation selection × output config);
...
re-entry (revisit trigger recorded in §7; superseded 2026-09-21 by ADR
0004's plan-time resolution of stable `$dynamicRef` sites, which removes
the islands rather than re-entering them).
...
2020-12 cross-referencing in one engine, both directions — the downstream
project's shape). Built against core's public surface only, zero new core API
...
chain asserted all-static + $dynamicRef island — since ADR 0004 an
unstable-site island, stable sites being compiled — non-vacuity checked)
...
1. **M10 — @json-schema-engine/dialect-draft04 (top priority: the downstream project needs it).**
   ...
   2020-12 resources cross-referencing in one engine — the downstream
   project's shape). Error params: sweep draft-04 keyword error sites to the M8.1
...
standalone, D9d thresholds, island re-entry (superseded by ADR 0004,
2026-09-21: stable `$dynamicRef` sites resolve at plan time), artifact registry-snapshot
semantics across interpreter trampolines (DELIVERED 2026-09-06, E1: every
artifact binds to `SchemaRegistry.snapshot()` — a read-only view over a
`DialectRegistry.snapshot()`, shared copy-on-write until the source
registers again — so a reference unresolved at compile time stays
unresolved for that artifact and re-registered resources or dialects never
reach it; regression `packages/compiler/test/registry-snapshot.test.ts`;
standalone was already closed-world),
code.source mapping,
...
annotation units, 160k dropped errors). Compiled structured documents,
step 1 — DELIVERED 2026-09-06: the document renderers consume a
...
Cross-repository command, package, TypeScript, and package-manager alignment
is tracked by the downstream project's tooling-convergence plan.
JSE's next infrastructure step there is migration from npm workspaces to pnpm while preserving every
current gate and the downstream project's tarball-vendoring boundary.
